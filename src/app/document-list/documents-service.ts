...
export class DocumentService {
  ...
  patchDocument(data: ModifyDocumentDTO, id: string) {
    return this.httpClient.patch<DocumentDTO>(
      environment.baseUrl + ApiPaths.Document + '/' + id,
      data
    );
  }
  ...
  copyDocuments(ids: string[], parentFolderId: string) {
    return this.httpClient.post<DocumentDTO[]>(
      environment.baseUrl + ApiPaths.DocumentCopy,
      {
        documents: ids,
  ...
  cutDocuments(ids: string[], parentFolderId: string) {
    return this.httpClient.post<DocumentDTO[]>(
      environment.baseUrl + ApiPaths.DocumentCut,
      {
        documents: ids,
        folder_id: parentFolderId
      }
  ...
  versionDocument(id: string) {
    return this.httpClient.post<DocumentDTO>(
      environment.baseUrl + ApiPaths.DocumentVersion + '/' + id,
      {}
    );
  }
  ...
  branchDocument(id: string) {
    return this.httpClient.post<DocumentDTO>(
      environment.baseUrl + ApiPaths.DocumentBranch + '/' + id,
      {}
    );
  }
  ...
  getVersions(rootId: string) {
    return this.httpClient.get<DocumentDTO[]>(
      environment.baseUrl + ApiPaths.DocumentVersions + '/' + rootId
    );
  }
}
